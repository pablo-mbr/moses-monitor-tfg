...
from core.communication.tapo_client import measure_consumption
from core.communication.mqtt_listener import init_mqtt_listener
...
def manage_plug_threads(nuevos_datos, q):
    ...
    def run_engine():
        ...
        try:
            ...
            loop.close()
            main_win.terminal.log("[MAIN] Bucle de motor Tapo cerrado.", LogType.INFO)
    
    # 4. Lanzamos el hilo como daemon para que muera si cerramos la App
    hilo = threading.Thread(target=run_engine, daemon=True)
    hilo.start()
    main_win.terminal.log(f"[MAIN] Hilo Tapo reiniciado para IP: {nuevos_datos.get('ip')}.", LogType.INFO)
...
if __name__ == "__main__":
    ...
    if config.exec() == QDialog.DialogCode.Accepted:
        ...
        main_win = MainWindow(start_data["inst_threshold"], start_data["accum_threshold"], start_data["baseline"], data_queue)
