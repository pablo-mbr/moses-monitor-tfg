...
from colors import GREEN, PURPLE, RED, ORANGE, WHITE
...
class AppLayout(QMainWindow):
    def setup_ui(self, model):
        ...
        # 4. Grupo Umbrales y Early Stopping
        self.group_data = QGroupBox("Umbrales y Alertas", objectName="GroupData")
        layout_data = QGridLayout(self.group_data)

        self.input_inst_threshold = QLineEdit("" if model.inst_threshold is None else str(model.inst_threshold))
        self.input_accum_threshold = QLineEdit("" if model.accum_threshold is None else str(model.accum_threshold))
        self.input_baseline = QLineEdit("" if model.baseline is None else str(model.baseline))
        ...
        self.graph_training_metrics.setBackground(WHITE)
